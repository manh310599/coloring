...
import 'package:coloring/src/queuelinear_floodfiller.dart';
...
class FloodFillPainter extends CustomPainter {
  ...
  void _initFloodFiller() async {
    ...
    _filler = QueueLinearFloodFiller(decoded, img.rgbaToUint32(fillColor.red, fillColor.green, fillColor.blue, fillColor.alpha));
    onInitialize!();
  }
  ...
  void setFillColor(Color color) {
    _filler?.setFillColor(img.rgbaToUint32(color.red, color.green, color.blue, color.alpha));
  }
  ...
  bool _isAvoidColor(Color avoidColor, int touchColor) {
    int touchR = img.uint32ToRed(touchColor);
    int touchG = img.uint32ToGreen(touchColor);
    int touchB = img.uint32ToBlue(touchColor);
    int touchA = img.uint32ToAlpha(touchColor);
  ...
  void fill(Offset position) async {
    ...
    int touchColor = _filler!.image!.getPixelIndex(pX, pY);
