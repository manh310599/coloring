...
class QueueLinearFloodFiller {
  ...
  void setTargetColor(int targetColor) {
    _startColor[0] = img.uint32ToRed(targetColor);
    _startColor[1] = img.uint32ToGreen(targetColor);
    _startColor[2] = img.uint32ToBlue(targetColor);
    _startColor[3] = img.uint32ToAlpha(targetColor);
  }
  ...
  Future<void> floodFill(int x, int y) async {
    ...
    if (_startColor[0] == 0) {
      // ***Get starting color.
      int startPixel = image!.getPixelIndex(x, y);

      _startColor[0] = img.uint32ToRed(startPixel);
      _startColor[1] = img.uint32ToGreen(startPixel);
      _startColor[2] = img.uint32ToBlue(startPixel);
    }
  ...
  void _linearFill(int x, int y) {
    ...
    while (true) {
      ...
      image?.setPixelIndex(lFillLoc, y, _fillColor);
    ...
    while (true) {
      // **fill with the color
      image?.setPixelIndex(rFillLoc, y, _fillColor);
  ...
  bool _checkPixel(int x, int y) {
    int pixelColor = image!.getPixelIndex(x, y);
    int red = img.uint32ToRed(pixelColor);
    int green = img.uint32ToGreen(pixelColor);
    int blue = img.uint32ToBlue(pixelColor);
    int alpha = img.uint32ToAlpha(pixelColor);
